...
### 2025-01-25
- 学習プラン作成
- Phase 1.1 完了: wire と reg の違いを理解
  - `wire`: assign文でのみ代入可。値を保持しない（配線）
  - `reg`: always文でのみ代入可。クロックまで値を保持（記憶素子）
  - 代入「元」は何でもOK、制限があるのは代入「先」だけ
- 次回: Phase 1.2 (assign文を使いこなす) から

### 2025-02-01
- Phase 1.2 完了: assign文の基本を理解
  - 基本論理演算子: `&`(AND), `|`(OR), `^`(XOR), `~`(NOT)
  - 三項演算子 `sel ? a : b` でMUXを記述できる
  - assign文は常にwireに対して使う（クロック不要の組み合わせ回路）
- Phase 1.3 開始: 2入力MUX を作成・シミュレーション完了
  - `learning/phase1/mux2.v`, `learning/phase1/tb_mux2.v`
  - ポートの型: `input`は常にwire、`output`はデフォルトwire（`output reg`で変更可）
- 次回: 4入力MUX の課題に進む
